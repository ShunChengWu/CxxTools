...
namespace caffe2 {
    ...
    std::string filename_to_key(const std::string &filename) {
        // return filename;
        return std::to_string(std::hash<std::string>{}(filename)) + "_" + filename;//使用hash獲得一個特定的值。輸入一樣的argument，hash必定給予一樣的
    }
    ...
    void write_batch(Workspace &workspace, NetBase *predict_net,
        ...
        std::unique_ptr<db::Transaction> transaction[kRunNum];//用來寫和確認寫入
        ...
        TensorCPU input;
        TensorUtil(input).ReadImages(filenames, size_to_fit, indices);//indices:儲存每張圖的大小(W*H)
        TensorCPU output;
        if (predict_net) {//給輸入並且run，紀錄輸出 （？）假如有多項輸出？剛好是分岔點之類
            ...
            output.ResizeLike(input);//如果predict net 是沒有output的那就直接拿input當作output
        ...
        data->set_data_type(TensorProto::FLOAT);//感覺這裡要能夠選擇資料型態
        label->set_data_type(TensorProto::INT32);//感覺這裡要能夠選擇資料型態
        ...
        for (auto i : indices) {//依據每個影像的dimension給予輸入（感覺可以為固定值，因為輸入大小不可能變動）
            auto single = TensorCPU(dims, std::vector<float>(output_data, output_data + size), NULL);
            output_data += size;
            data->Clear();
            serializer.Serialize(single, "", data, 0, kDefaultChunkSize);//將 single data serialize
            label->set_int32_data(0, batch_files[i].second);
            protos.SerializeToString(&value);//準備寫入db
            int percentage = 0, p = (int)(rand() * 100.0 / RAND_MAX);//這裡把資料分為3部分：test, validate, train
            auto key = filename_to_key(batch_files[i].first);//使用hash獲得unique key，可能用在避免寫入一模一樣的資料兩次，與pre_process的key相呼應
    ...
    /*!
     * @discussion 傳入前半部分的net進行前處理
     * @discussion對輸入的影像做運算，輸出前半部分net輸出的結果並儲存成後半部分的輸入
     * @param <#param description#>
     * @return <#return description#>
     */
    void pre_process(const std::vector<std::pair<std::string, int>> &image_files, const std::string *db_paths, NetDef &init_model, NetDef &predict_model, const std::string &db_type, int batch_size, int size_to_fit) {
        ...
        std::unique_ptr<db::DB> database[kRunNum];//kRunNum=3
        for (int i = 0; i < kRunNum; i++) {
        ...
        init_net->Run();
        auto predict_net = predict_model.external_input_size()? CreateNet(predict_model, &workspace) : NULL; //有輸入才創建
        ...
        for (auto &pair : image_files) {
            ...
            image_count++;
            auto in_db = false;
            auto key = filename_to_key(filename);//使用hash值當作key，在write_batch也用相同的key來儲存
            ...
            if (batch_files.size() == batch_size) { //run only when batch.size reach batch_size
                std::cerr << '\r' << std::string(40, ' ') << '\r' << "pre-processing.. "
                << image_count << '/' << image_files.size() << " "
                << std::setprecision(3)
                << ((float)100 * image_count / image_files.size()) << "%"
                << std::flush;//flush跟cerr都是讓輸出更快更直接避免錯誤的方法
                //把輸入的資料用write_batch寫道
                write_batch(workspace, predict_net ? predict_net.get() : NULL, input_name, //Run predict net & write
    ...
    void split_model(NetDef &base_init_model, NetDef &base_predict_model,
        ...
        std::set<std::string> static_inputs = NetUtil(base_predict_model).CollectLayers(layer);//獲得該layer以前的layers
        ...
        //COPY external_output from init
        ...
        //COPY external_input from predict
        ...
        //增加所指定的輸出layer作為first_model的output
        if (first_predict_model.op().size()) {
            first_predict_model.add_external_output(layer);
        }
        //若有external output就新增
        if (second_predict_model.op().size()) {
            std::vector<std::string> external_output (base_predict_model.external_output().begin(), base_predict_model.external_output().end());
            for (auto &output: external_output) second_predict_model.add_external_output(output);
        }
        
        //假如有名字則將其分為first & second
    ...
    void add_train_model(NetDef &base_init_model, NetDef &base_predict_model,
        ...
        std::string last_w, last_b;//用於偵測最後一層
        for (const auto &op : base_predict_model.op()) {
            auto new_op = train_predict_model.add_op();
            new_op->CopyFrom(op);
            set_trainable(*new_op, true);//這裡要新增SBN，看還沒有沒其他需要的
            if (op.type() == "FC") {//這裡假設最後一層一定是FC，所以持續update w,b 最後會獲得最後一層的w, b
                last_w = op.input(1);
                last_b = op.input(2);
            }
        }
        ...
        for (const auto &op : base_init_model.op()) {
            ...
            bool uniform = (output.find("_b") != std::string::npos);//npos: until the end of the string
            //這裡假設結尾為“_b”型態的就是使用ConstantFill
